...
# アイテムが入っているスロットを取得
execute store result score #get_inventory.slot rukky.func run data get entity @s Inventory[{tag:{Trim:{material:"extract_enchantments:all"}}}].Slot
...
#エンチャ本が対象だった場合
execute if data storage rukky-func get_inventory.data.tag.StoredEnchantments run function extract_enchantments:modify/eb_all
#それ以外が対象だった場合
